...
- **Alert evaluation is skipped** for that node. No false "Node Unreachable" or "Agent Heartbeat Lost" alerts fire while metrics collection is still ramping up.
...
- The installer now detects whether it's running on a fresh install or an upgrade. On upgrade, it preserves existing database credentials, SSL certificates, and service registrations instead of regenerating them.
- Backend and NGINX services are stopped automatically before the installer replaces binaries, preventing file-locking conflicts.
- Services are gracefully stopped and restarted during the standard MSI upgrade sequence on all future version transitions.
...
- The stabilization check is now a single helper driven by a per-node `stabilization_until` timestamp, replacing several copy-pasted grace-period blocks scattered across the alert evaluator. Behavior is unchanged for customers; this is internal consolidation that prevents future drift.
- Every alert creation path — including DNS resolution checks that previously took an independent code path — now consults the stabilization timestamp, so a brand-new node doesn't get paged on conditions that depend on data not yet collected.
